...
def evaluate(
    ...
    # For livecodebench, skip evaluation and just return throughput metrics
    if data_name == "livecodebench":
        # Add dummy scores for livecodebench
        for sample in samples:
            sample["gt_cot"], sample["gt"] = None, "N/A"
            # Set dummy scores (not used for livecodebench)
            sample["score"] = [True] * len(sample.get("pred", [1]))

        result_json = {
            "num_samples": len(samples),
            "num_scores": sum(len(s.get("pred", [1])) for s in samples),
            "timeout_samples": 0,
            "empty_samples": 0,
            "acc": 0.0,  # Not applicable for livecodebench
            "note": "LiveCodeBench: evaluation skipped, only throughput recorded",
        }
        print(result_json)
